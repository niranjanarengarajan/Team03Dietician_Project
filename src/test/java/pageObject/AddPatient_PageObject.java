...
public class AddPatient_PageObject {
    ...
    public void enterFieldValue(String field, String value) {
        ...
        switch (field.toLowerCase()) {
            case "first name": element = firstName; break;
            case "last name": element = lastName; break;
            case "weight": element = weight; break;
            case "height": element = height; break;
            case "temperature": element = temperature; break;
            case "sp": element = sp; break;
            case "dp": element = dp; break;
    ...
    public void skipDropdown(String field) {
        WebElement element;
        switch (field.toLowerCase()) {
            case "allergies": element = allergyDropdown; break;
            case "food preference": element = foodDropdown; break;
            case "cusine category": element = cuisineDropdown; break;
            default: throw new IllegalArgumentException("Unknown dropdown: " + field);
        }
        waitUtils.waitForVisibility(driver, element, 10);
        element.sendKeys("\t"); 
    }
    ...
    public void uploadFile(String fileType) {
        ...
        switch (fileType.toLowerCase()) {
            case "pdf": path = "src/test/resources/files/sample.pdf"; break;
            case "docx": path = "src/test/resources/files/sample.docx"; break;
            case "jpeg": path = "src/test/resources/files/sample.jpeg"; break;
            case "largefile": path = "src/test/resources/files/largefile.pdf"; break;
            case "none": return;
    ...
    public void createPatientFromExcel() {
        ...
        if (data.containsKey("contact_number")) enterContact(data.get("contact_number"));
        if (data.containsKey("dob")) enterDOB(data.get("dob"));
        if (data.containsKey("weight")) enterFieldValue("weight", data.get("weight"));
        if (data.containsKey("height")) enterFieldValue("height", data.get("height"));
        if (data.containsKey("temperature")) enterFieldValue("temperature", data.get("temperature"));
        if (data.containsKey("sp")) enterFieldValue("sp", data.get("sp"));
        if (data.containsKey("dp")) enterFieldValue("dp", data.get("dp"));
        if (data.containsKey("allergy")) selectAllergy(data.get("allergy"));
        if (data.containsKey("food_preference")) selectFoodPreference(data.get("food_preference"));
        if (data.containsKey("cuisine_category")) selectCuisineCategory(data.get("cuisine_category"));
        if (data.containsKey("file_type")) uploadFile(data.get("file_type"));
    ...
    public boolean verifyPatientCreatedFromExcel() {
        ...
        for (WebElement msg : messageText) {
            if (msg.getText().contains("Patient successfully created")) return true;
        }
        return false;
    }
